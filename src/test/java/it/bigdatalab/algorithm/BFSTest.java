...
import it.bigdatalab.applications.randomBFS;
...
public class BFSTest {
    ...
    @ParameterizedTest(name = "{index} => direction={0}, seeds={1}, nodes={2}, expected={3}")
    @MethodSource("pathProvider")
    void testAlgorithm_DiPath_Old(String direction, int[] seeds, int[] nodes, Measure expected) throws IOException, MinHash.SeedsException {
        String path = new File("src/test/data/g_directed/32-path.graph").getAbsolutePath();
        path = path.substring(0, path.lastIndexOf('.'));
        Parameter param = new Parameter.Builder()
                .setInputFilePathGraph(path)
        ...
        randomBFS algo = new randomBFS(param);
        List<Measure> measure = algo.run();
    ...
    @ParameterizedTest(name = "{index} => direction={0}, seeds={1}, nodes={2}, expected={3}")
    @MethodSource("unWheelProvider")
    void testAlgorithm_UnWheel_Old(String direction, int[] seeds, int[] nodes, Measure expected) throws IOException, MinHash.SeedsException {
        String path = new File("src/test/data/g_undirected/32-wheel.graph").getAbsolutePath();
        path = path.substring(0, path.lastIndexOf('.'));
        Parameter param = new Parameter.Builder()
                .setInputFilePathGraph(path)
