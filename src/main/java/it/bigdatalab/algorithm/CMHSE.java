...
public class CMHSE extends CMinHash {

    public static final Logger logger = LoggerFactory.getLogger("it.bigdatalab.algorithm.CMHSE");
     ...
     */
    public CMHSE(final CompressedGraph g, int numSeeds, double threshold, IntArrayList seeds) throws CMinHash.SeedsException {
        super(g, numSeeds, threshold, seeds);

        signatures = new Int2ObjectOpenHashMap<>(cGraph.numNodes());       //initialize signatures map with the expected number of elements(nodes) in the map
        oldSignatures = new Int2ObjectOpenHashMap<>(cGraph.numNodes());
     ...
     * Creates a new MHSE instance with default values
     */
    public CMHSE(final CompressedGraph g, int numSeeds, double threshold) throws CMinHash.SeedsException {
    ...
    public Measure runAlgorithm() {
        ...
        while (signatureIsChanged) {
            ...
            if (signatureIsChanged) {
                double estimatedCouples = overallJaccard * cGraph.numNodes();
        ...
        graphMeasure.setNumNodes(cGraph.numNodes());
        graphMeasure.setNumArcs(cGraph.numArcs());
    ...
    public boolean updateNodeSignature(int node) {
        ...
        // mettere true come parametro nel properties
        int [] neigh = cGraph.get_neighbours(node,true);
        int k;
        int d = neigh.length;
